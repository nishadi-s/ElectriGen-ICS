...
router
  .route("/add")
  .post(authMiddleware([USER_ROLES.DONATION_MANAGER]), async (req, res) => {
    try {
...
router
  .route("/")
  .get(authMiddleware([USER_ROLES.DONATION_MANAGER]), async (req, res) => {
    try {
...
router
  ...
  .put(authMiddleware([USER_ROLES.DONATION_MANAGER]), async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json({ status: "Successfully updated", project: updatedProject });
    } catch (err) {
      console.error(err);
...
router
  ...
  .delete(authMiddleware([USER_ROLES.DONATION_MANAGER]), async (req, res) => {
    const project_id = req.params.project_id;
